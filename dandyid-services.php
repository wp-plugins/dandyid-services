...
// Constants
define ('DANDYID_PLUGIN_VERSION',         'DandyID-Services-v1.5.9');
...
function dandyIDServices_getTable ()
    ...
    for ($i = 0; $cacheOptions [$i] ['url'] != ''; $i++)
        ...
        if ($dandyID_settingsOptions ['show_style'] == DANDYID_SHOW_FAVICONS_AND_TEXTLINKS)
            ...
            $buf .= sprintf ("<a href=\"%s\" rel=\"me\" target=\"_blank\">", $cacheUrl);
            $buf .= sprintf ("<img src=\"%s\" title=\"%s\" width=\"16\" height=\"16\" alt=\"%s\" /></a> ",
                              $cacheSvcFavicon, $cacheSvcName, $cacheSvcName);
            }
    ...
    if ($dandyID_settingsOptions ['show_powered_by'] == TRUE)
        ...
        if (($dandyID_settingsOptions ['show_style'] == DANDYID_SHOW_FAVICONS_AND_TEXTLINKS) ||
            ...
            $buf .= '<br /><span style="font-size:.75em;"><a href="' . DANDYID_URL . '">Powered by DandyID</a></span>';
            }
        }
