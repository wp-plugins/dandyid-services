...
// Constants
define ('DANDYID_PLUGIN_VERSION',         'DandyID-Services WordPress Plugin v1.4.6');
...
function dandyIDServices_getTable ()
    ...
    for ($i = 0; $cacheOptions [$i] ['url'] != ''; $i++)
        ...
        if ($dandyID_settingsOptions ['show_style'] == DANDYID_SHOW_FAVICONS_AND_TEXTLINKS)
            ...
            $buf .= '<li>';
            $buf .= sprintf ("<a href=\"%s\" title=\"%s\" rel=\"me\">", $cacheUrl, $cacheSvcName);
            ...
            $buf .= '<li>';
            $buf .= sprintf ("<a href=\"%s\" title=\"%s\" rel=\"me\">%s</a>", $cacheUrl, $cacheSvcName, $cacheSvcName);
            $buf .= '</li>';
            }
        }
