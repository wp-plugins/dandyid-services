...
// Constants
define ('DANDYID_PLUGIN_VERSION',         'DandyID-Services-v1.4.9');
...
function dandyIDServices_getTable ()
    ...
    for ($i = 0; $cacheOptions [$i] ['url'] != ''; $i++)
        ...
        if ($dandyID_settingsOptions ['show_style'] == DANDYID_SHOW_FAVICONS_AND_TEXTLINKS)
            ...
            $buf .= '<li>';
            $buf .= sprintf ("<a href=\"%s\" title=\"%s\" rel=\"me\" target=\"_blank\">", $cacheUrl, $cacheSvcName);
            ...
            $buf .= '<li>';
            $buf .= sprintf ("<a href=\"%s\" title=\"%s\" rel=\"me\" target=\"_blank\">%s</a>", $cacheUrl, $cacheSvcName, $cacheSvcName);
            $buf .= '</li>';
            }
        }
...
function dandyIDServices_initWidget ()
    ...
    if (function_exists ('register_sidebar_widget'))
        ...
        function dandyIDServices_widget ($args)
            ...
            printf ("<div id=\"%s\">", DANDYID_PLUGIN_VERSION);
